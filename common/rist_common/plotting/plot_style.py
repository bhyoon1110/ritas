...
#{div_id} .rist-legend-bulk-button.is-active {{
  border-color: #3b82f6;
  background: #dbeafe;
  color: #1d4ed8;
}}
...
(function() {{
  ...
  function dispatchVisibilityChange(curves, visible, userVisible) {{
    try {{
      var detail = {{ curves: curves, visible: visible }};
      if (typeof userVisible === "boolean") detail.userVisible = userVisible;
  ...
  function refreshPeakLabelStateFromLayout() {{
    var layoutMeta = gd.layout && gd.layout.meta && typeof gd.layout.meta === "object"
      ? gd.layout.meta : {{}};
    gd._ristPeakLabelsVisible = layoutMeta.ristPeakLabelsVisible !== false;
    return gd._ristPeakLabelsVisible;
  }}

  function peakLabelsVisible() {{
    if (typeof gd._ristPeakLabelsVisible !== "boolean") {{
      return refreshPeakLabelStateFromLayout();
    }}
    return gd._ristPeakLabelsVisible;
  }}

  function updatePeakLabelButton(button) {{
    if (!button) return;
    var visible = peakLabelsVisible();
    button.classList.toggle("is-active", visible);
    button.setAttribute("aria-pressed", visible ? "true" : "false");
    button.title = visible ? "그래프 피크 라벨 숨김" : "그래프 피크 라벨 표시";
  }}

  function setPeakLabelVisibility(visible, button) {{
    if (!window.Plotly) return;
    if (gd._ristHistory) gd._ristHistory.capture();
    gd._ristPeakLabelsVisible = !!visible;
    var layoutMeta = Object.assign(
      {{}},
      gd.layout && gd.layout.meta && typeof gd.layout.meta === "object"
        ? gd.layout.meta : {{}}
    );
    layoutMeta.ristPeakLabelsVisible = !!visible;
    window.Plotly.relayout(gd, {{ meta: layoutMeta }}).then(function() {{
      updatePeakLabelButton(button);
      gd.dispatchEvent(new CustomEvent("rist-peak-label-visibility-change", {{
        detail: {{ visible: !!visible }}
      }}));
    }});
  }}

  function refreshPeakBulkControls(bulk) {{
    var hasPeaks = false;
    for (var i = 0; i < (gd.data || []).length; i++) {{
      if (isPeakCurve(i)) {{
        hasPeaks = true;
...
(function() {{
  ...
  function peakUserVisible(curve) {{
    var state = gd._ristPeakUserVisibility;
    var meta = traceMeta(curve);
    var key = String(
      meta.rist_legend_edit_group
      || (meta.rist_peak && meta.rist_peak.label_key)
    ...
    return peak.user_visible !== false;
  }}
  ...
  function applySensitivity(sensitivity) {{
    ...
    labels.forEach(function(label) {{
      ...
      var on = visibleByCurve[label.traceIndex] && peakLabelsVisible();
...
(function() {{
  ...
  function refreshPeakLabelVisibility() {{
    var layoutMeta = gd.layout && gd.layout.meta && typeof gd.layout.meta === "object"
      ? gd.layout.meta : {{}};
    gd._ristPeakLabelsVisible = layoutMeta.ristPeakLabelsVisible !== false;
    return gd._ristPeakLabelsVisible;
  ...
  function peakLabelsVisible() {{
    if (typeof gd._ristPeakLabelsVisible !== "boolean") {{
      return refreshPeakLabelVisibility();
    }}
    return gd._ristPeakLabelsVisible;
  }}
  ...
  function syncVisibility() {{
    ...
    labels.forEach(function(label) {{
      var on = peakLabelsVisible()
        && traceVisible(label.traceIndex)
  ...
  function recordPeakUserVisibilityFromCustomEvent(ev) {{
    ...
    var desiredUserVisible = typeof detail.userVisible === "boolean"
      ? detail.userVisible : null;
    curves.forEach(function(curve, pos) {{
      if (!isPeakCurve(curve)) return;
      var visible = desiredUserVisible == null
        ? (Array.isArray(visibleValues) ? visibleValues[pos] : visibleValues)
        : desiredUserVisible;
      setPeakUserVisibility(curve, visible);
    }});
  }}
  ...
  gd.addEventListener("rist-peak-label-visibility-change", function(ev) {{
    var detail = ev.detail || {{}};
    gd._ristPeakLabelsVisible = detail.visible !== false;
    syncVisibility();
  }});
  ...
  function applyMode(enabled) {{
    ...
    if (window.Plotly) {{
      if (nextEnabled) {{
        ...
        modePromise = window.Plotly.relayout(gd, {{ dragmode: false }}).then(syncModebarState);
      }} else {{
        if (gd._context && previousScrollZoom !== null) {{
          gd._context.scrollZoom = previousScrollZoom;
        }}
        modePromise = window.Plotly.relayout(gd, {{
...
def _axis_controls_js(div_id: str) -> str:
    """축 범위·눈금, 확대·축소, 드래그 자르기 UI를 추가한다."""
    template = r"""
<style>
...
#__PLOT_ID_CSS__ .rist-axis-control-footer {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 10px;
}
#__PLOT_ID_CSS__ .rist-axis-control-view-actions {
  display: flex;
  align-items: center;
  gap: 6px;
}
#__PLOT_ID_CSS__ .rist-axis-control-view-actions button {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  flex: 0 0 34px;
...
#__PLOT_ID_CSS__ .rist-axis-control-view-actions button:hover {
  border-color: #3b82f6;
  background: #eaf4ff;
  color: #075985;
}
#__PLOT_ID_CSS__ .rist-axis-control-view-actions svg {
  width: 18px;
  height: 18px;
  pointer-events: none;
}
...
#__PLOT_ID_CSS__.rist-axis-crop-mode .draglayer {
  cursor: crosshair !important;
}
#__PLOT_ID_CSS__ .rist-axis-crop-hint {
  position: absolute;
  z-index: 73;
  top: 70px;
  left: 50%;
...
#__PLOT_ID_CSS__ .rist-axis-crop-hint[hidden] {
  display: none !important;
}
...
(function() {
  ...
  cropHint.className = "rist-axis-crop-hint";
  cropHint.hidden = true;
  cropHint.textContent = "자를 영역을 그래프에서 드래그하세요. Esc를 누르면 취소됩니다.";
  gd.appendChild(cropHint);
  ...
  function zoomByFactor(factor) {
    ...
    var status = panel.querySelector(".rist-axis-control-status");
    if (!changed) {
      status.textContent = "확대할 축 범위를 확인하세요.";
      return;
    }
    if (gd._ristHistory && gd._ristHistory.capture) gd._ristHistory.capture();
    window.Plotly.relayout(gd, updates).then(function() {
  ...
  panel.querySelector("[data-axis-action='zoom-in']").addEventListener("click", function() {
    zoomByFactor(0.8);
  });
  panel.querySelector("[data-axis-action='zoom-out']").addEventListener("click", function() {
    zoomByFactor(1.25);
  });
  ...
  function normalizedCropRange(axis, values) {
    ...
    var from = finiteNumber(values[0]);
    var to = finiteNumber(values[1]);
    if (from == null || to == null || from === to) return null;
    var current = currentRange(axis + "axis");
    if (current && (current[1] - current[0]) * (to - from) < 0) {
      var swapped = from;
      from = to;
      to = swapped;
    }
  ...
  function restoredDragmode(session) {
    var dragmode = session && session.previousDragmode;
    if (!dragmode || dragmode === "select" || dragmode === "lasso") return "zoom";
    return dragmode;
  }
  ...
  function finishCropUi(session) {
    ...
    cropHint.hidden = true;
    return window.Plotly.relayout(gd, {
      dragmode: restoredDragmode(session),
      selectdirection: session && session.previousSelectdirection
        ? session.previousSelectdirection
        : "any",
      selections: []
    });
  }
  ...
  function beginCrop() {
    ...
    Promise.resolve(editPromise).then(function() {
      ...
      cropSession = {
        axes: axes,
        previousDragmode: previousDragmode,
        previousSelectdirection: previousSelectdirection
      };
      gd.classList.add("rist-axis-crop-mode");
