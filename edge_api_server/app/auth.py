...
def _admin_page() -> str:
    body = """
<style>
.admin-shell{width:100%;max-width:none;margin:0}.admin-top{min-height:64px;background:#fff;border-bottom:1px solid #d8dee8;display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap;padding:0 24px}.admin-top h1{font-size:21px;margin:0}.admin-content{width:100%;padding:20px 24px 40px}.top-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}.top-actions a,.logout-button{display:inline-flex;align-items:center;justify-content:center;min-height:38px;border:1px solid #bcc7d6;border-radius:6px;background:#fff;color:#42526b;padding:7px 12px;text-decoration:none;font:inherit;font-weight:700;cursor:pointer}.logout-button:disabled{opacity:.55;cursor:wait}.admin-tabs{display:flex;gap:4px;overflow-x:auto;border-bottom:1px solid #d8e0e8;margin:0 0 16px}.admin-tab{padding:11px 16px;color:#657286;text-decoration:none;font-weight:700;white-space:nowrap;border-bottom:3px solid transparent}.admin-tab.active{color:#183153;border-color:#2563eb}.admin-panel{background:#fff;border:1px solid #d8e0e8;border-radius:8px;padding:18px}.admin-summary{display:grid;grid-template-columns:repeat(5,minmax(100px,1fr));gap:9px;margin:16px 0}.summary-card{background:#f6f8fb;border:1px solid #d8e0e8;border-radius:6px;padding:12px}.summary-card strong{display:block;font-size:21px}.summary-card span{font-size:12px;color:#687587}.admin-filter{display:grid;grid-template-columns:minmax(240px,1fr) 180px auto;gap:8px}.admin-note{background:#f2f7fc;border-left:4px solid #2879bd;padding:12px;margin:14px 0;color:#405069}.admin-users{display:grid;gap:12px}.member-card{border:1px solid #d8e0e8;border-radius:7px;padding:15px}.member-head{display:flex;justify-content:space-between;gap:12px}.member-title{font-size:18px;font-weight:700}.badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}.badge{font-size:12px;padding:4px 8px;border-radius:999px;background:#edf2f7}.badge.active,.badge.sso{background:#dcfce7;color:#166534}.badge.pending{background:#fef3c7;color:#92400e}.badge.suspended{background:#fee2e2;color:#9b1c1c}.member-controls{display:grid;grid-template-columns:220px 1fr 1fr;gap:10px;margin-top:13px}.control{border:1px solid #e1e7ef;border-radius:6px;padding:11px}.control h3,.control legend{font-size:14px;font-weight:700}.checks{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.checks label{display:flex;align-items:center;gap:5px}.sso-detail{margin-top:10px;background:#f8fafc;border:1px solid #e1e7ef;border-radius:6px;padding:11px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.sso-detail b{display:block;font-size:11px;color:#687587;margin-bottom:3px}.member-actions{display:flex;justify-content:flex-end;align-items:center;gap:8px;margin-top:11px}.save-result{color:#166534;font-size:13px}.empty{text-align:center;padding:30px;color:#687587}
@media(max-width:800px){.admin-top{padding:12px 14px;align-items:flex-start}.admin-top .top-actions{width:100%}.admin-top h1{font-size:19px}.admin-content{padding:14px}.admin-summary{grid-template-columns:repeat(2,1fr)}.admin-filter,.member-controls,.sso-detail{grid-template-columns:1fr}.member-head{display:block}.admin-panel{padding:13px}}
</style>
<div class="admin-shell">
  ...
  <section class="admin-panel">
    ...
    <div id="users" class="admin-users"></div>
  </section>
  </div>
</div>
<script>
...
const usersEl=document.getElementById('users'),message=document.getElementById('message'),search=document.getElementById('member-search'),statusFilter=document.getElementById('status-filter');let members=[];
...
function render(){const term=search.value.trim().toLowerCase(),filter=statusFilter.value;const visible=members.filter(u=>{const text=[u.loginId,u.displayName,u.email,u.sso?.employee_id,u.sso?.display_name].filter(Boolean).join(' ').toLowerCase();return(!term||text.includes(term))&&(!filter||u.status===filter)});if(!visible.length){usersEl.innerHTML='<div class="empty">조건에 맞는 회원이 없습니다.</div>';return}usersEl.innerHTML=visible.map(u=>{const ssoActive=Boolean(u.sso&&u.sso.active),ssoText=!u.sso?'SSO 미연결 · 전송 불가':ssoActive?'SSO 연결·활성':'SSO 연결 중지 · 전송 불가';return `<article class="member-card" data-id="${esc(u.userId)}"><div class="member-head"><div><div class="member-title">${esc(u.displayName||u.loginId)}</div><span class="muted">로그인 ID ${esc(u.loginId)}${u.email?' · '+esc(u.email):''}</span><div class="badges"><span class="badge ${badgeClass(u.status)}">${esc(statusLabel[u.status]||u.status)}</span><span class="badge ${ssoActive?'sso':'suspended'}">${esc(ssoText)}</span>${(u.roles||[]).map(x=>`<span class="badge">${esc(roleLabel[x]||x)}</span>`).join('')}</div></div><span class="muted">가입 ${date(u.createdAt)}<br>최근 로그인 ${date(u.lastLoginAt)}</span></div><div class="member-controls"><div class="control"><h3>승인 상태</h3><select class="status">${Object.keys(statusLabel).map(x=>`<option value="${x}" ${x===u.status?'selected':''}>${statusLabel[x]}</option>`).join('')}</select></div><fieldset class="control"><legend>프로젝트 접근 및 보고서 생성</legend><div class="checks">${projects.map(x=>`<label><input class="project" type="checkbox" value="${x}" ${(u.projects||[]).includes(x)?'checked':''}> ${x}</label>`).join('')}</div></fieldset><fieldset class="control"><legend>운영·전송 역할</legend><div class="checks">${roles.map(x=>`<label><input class="role" type="checkbox" value="${x}" ${(u.roles||[]).includes(x)?'checked':''}> ${roleLabel[x]}</label>`).join('')}</div></fieldset></div><div class="sso-detail"><div><b>SSO 상태</b>${esc(ssoText)}</div><div><b>제공자 / 사번</b>${esc(u.sso?.provider||'-')} / ${esc(u.sso?.employee_id||'-')}</div><div><b>SSO 사용자</b>${esc(u.sso?.display_name||u.sso?.email||'-')}</div><div><b>최근 SSO 인증</b>${date(u.sso?.last_authenticated_at)}</div></div><div class="member-actions"><span class="save-result"></span><button type="button" class="save">변경사항 저장</button></div></article>`}).join('')}
async function load(){message.hidden=true;usersEl.innerHTML='<div class="empty">회원 목록을 불러오는 중입니다.</div>';const r=await fetch('/api/v1/auth/admin/users');const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||'회원 목록을 불러오지 못했습니다.');members=d.items||[];updateSummary();render()}
async function save(btn){const el=btn.closest('.member-card'),result=el.querySelector('.save-result'),body={status:el.querySelector('.status').value,projects:[...el.querySelectorAll('.project:checked')].map(x=>x.value),roles:[...el.querySelectorAll('.role:checked')].map(x=>x.value)};btn.disabled=true;result.textContent='저장 중...';try{const r=await fetch('/api/v1/auth/admin/users/'+encodeURIComponent(el.dataset.id),{method:'PATCH',headers:{'Content-Type':'application/json','X-Requested-With':'RIST-Admin'},body:JSON.stringify(body)});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||'저장하지 못했습니다.');result.textContent='저장했습니다.';await load()}catch(e){message.textContent=e.message;message.className='error';message.hidden=false;result.textContent=''}finally{btn.disabled=false}}
...
usersEl.addEventListener('click',e=>{const btn=e.target.closest('.save');if(btn)save(btn)});search.addEventListener('input',render);statusFilter.addEventListener('change',render);document.getElementById('reload').addEventListener('click',()=>load().catch(showError));document.getElementById('logout-button').addEventListener('click',logout);function showError(e){message.textContent=e.message;message.className='error';message.hidden=false}load().catch(showError);
</script>"""
