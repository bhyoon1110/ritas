...
usersEl.addEventListener('click',e=>{const btn=e.target.closest('.save');if(btn)save(btn)});search.addEventListener('input',render);statusFilter.addEventListener('change',render);document.getElementById('reload').addEventListener('click',()=>load().catch(showError));function showError(e){message.textContent=e.message;message.className='error';message.hidden=false}load().catch(showError);
</script>"""
