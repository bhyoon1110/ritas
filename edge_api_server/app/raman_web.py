...
.raman-report-progress.is-visible {
  display: block;
  position: fixed;
  left: 50%;
  top: 50%;
...
.raman-report-progress.is-error {
  border-color: #fecaca;
  background: #fef2f2;
  color: #991b1b;
}
...
.raman-report-progress.is-error .raman-report-progress-bar { background: #dc2626; }
#raman-plot {
  min-height: 540px;
...
(function() {
  ...
  function applyInitialYAxisDisplay() {
    applyOffsets({preserveView: true}).catch(function(error) {
      console.error("Raman 스택 초기 표시 반영 실패", error);
    });
  }
  ...
  function visualLegendOrder() {
    ...
    return groups.sort(function(left, right) {
      var delta = Number(state.offsets[right] || 0)
        - Number(state.offsets[left] || 0);
