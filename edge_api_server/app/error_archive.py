...
<body data-default-tab="__DEFAULT_TAB__">
  ...
  (function(){
    ...
    function setTab(tab,loadNow){activeTab=tab==='errors'?'errors':'usage';document.querySelectorAll('.tab').forEach(button=>button.classList.toggle('active',button.dataset.tab===activeTab));document.querySelectorAll('.usage-only').forEach(element=>element.classList.toggle('hidden',activeTab!=='usage'));$('state').innerHTML=activeTab==='usage'?'<option value="">전체 결과</option><option value="success">성공</option><option value="failure">실패</option>':'<option value="">전체 상태</option><option value="open">미해결</option><option value="resolved">해결</option>';$('head-row').innerHTML=activeTab==='usage'?'<th>발생 시각</th><th>프로젝트</th><th>기록 유형</th><th>동작</th><th>결과</th><th>처리시간</th><th>의뢰번호</th><th>작업 ID</th><th>클라이언트</th>':'<th>발생 시각</th><th>프로젝트</th><th>상태</th><th>오류 코드</th><th>메시지</th><th>파일</th>';$('detail').className='panel detail';$('empty').textContent=activeTab==='usage'?'사용 기록이 없습니다.':'오류 기록이 없습니다.';history.replaceState(null,'',activeTab==='usage'?'/operations':'/errors');if(activeTab==='usage')setDefaultDates();if(loadNow)load()}
