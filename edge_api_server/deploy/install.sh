...
if [[ ! -d "${PROJECT_ROOT}/common" || ! -d "${PROJECT_ROOT}/config" \
      || ! -d "${PROJECT_ROOT}/sune" || ! -d "${PROJECT_ROOT}/lim" \
      || ! -d "${PROJECT_ROOT}/rin" || ! -d "${PROJECT_ROOT}/ahn" \
      || ! -d "${EDGE_DIR}" ]]; then
    echo "    오류: ${PROJECT_ROOT} 아래에 common/ config/ sune/ lim/ rin/ ahn/ edge_api_server/ 폴더가 모두 있어야 합니다." >&2
    exit 1
fi
