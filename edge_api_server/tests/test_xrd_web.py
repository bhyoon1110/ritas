...
def test_xrd_workspace_contains_upload_controls() -> None:
    ...
    assert page.index('id="xrd-logout"') > page.index('id="xrd-download"')
...
def test_xrd_report_can_use_llm_comment_provider(tmp_path) -> None:
    ...
    assert "rist-xrd-legend-size-toggle" in result["html"]
    assert "rist-xrd-legend-hide" in result["html"]
    assert "rist-xrd-legend-restore" in result["html"]
    assert "범례 · 축소" in result["html"]
    assert "전체 범례 보기" in result["html"]
    assert 'gd.dataset.xrdLegendMode = mode' in result["html"]
