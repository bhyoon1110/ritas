...
def test_ftir_workspace_contains_upload_and_editor_controls() -> None:
    ...
    assert 'id="ftir-logout" hidden>로그아웃</button>' in page
    ...
    assert ".ftir-message-item" in page
...
def test_ftir_analysis_zero_sensitivity_disables_detected_peaks(tmp_path: Path) -> None:
    with TestClient(create_ftir_preview_app(tmp_path / "libraries")) as client:
        response = client.post(
            "/api/v1/ftir/analyze",
            files={
