...
사용 기록은 화면 접속과 실제 업무 수행을 구분한다. `보고서 생성 요청`은 작업을
접수한 기록이고, `보고서 완료`는 백그라운드 처리 후 보고서 패키지가 실제로
생성된 경우에만 별도 기록된다. 따라서 운영 화면의 기록 유형 필터에서 단순
화면 조회를 제외하고 실제 생성 완료 건수만 확인할 수 있다.

사용 기록 목록의 클라이언트 항목에는 C#/.NET 프로그램명, 실험 PC 이름과 접속
IP가 함께 표시된다. 리버스 프록시를 거치는 경우 `X-Forwarded-For`의 최초 접속
IP와 Edge 서버가 직접 연결받은 IP를 상세 화면에서 각각 확인할 수 있다.
