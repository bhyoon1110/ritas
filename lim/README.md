...
- 분석결과: raw 피크, ICDD 후보상, 첨부 표/이미지 정보를 기준으로 정리한
  분석 의견. HTML 화면에서 수정할 수 있으며 다운로드 시 수정사항이 반영된다.
...
Edge API 통합 서버에서 실행하면 분석결과 문안은 설정된 로컬 모델을 우선
사용하고, 호출 실패 시 규칙 기반 문안으로 대체된다.
